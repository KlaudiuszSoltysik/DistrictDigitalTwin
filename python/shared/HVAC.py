...
class HVAC:
    ...
    def step(self, current_time, dt, thermal_solver, co2_solver, weather_service, weather_solver):
        ...
        if needs_recalc:
            ...
            bounds_q = [(-100.0, 100.0) for _ in range(control_steps) for i in range(self.num_nodes)]
            bounds_v = [(0.0, 100.0) for _ in range(control_steps) for i in range(self.num_nodes)]
            ...
            half_idx = control_steps * self.num_nodes
            optimal_q_percent = res.x[:half_idx].reshape((control_steps, self.num_nodes))
